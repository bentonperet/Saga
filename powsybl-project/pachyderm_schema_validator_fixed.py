"""
PACHYDERM GLOBAL - Electrical Model Metadata Validator (FIXED)
Strict JSON schema validation with alerts and recommendations.
ADDED: Semantic validation to catch unreasonable values.
"""
...
# JSON Schema Definition (unchanged)
PACHYDERM_SCHEMA = {
...
class ValidationAlert:
    ...
    def __str__(self):
        icon = "X" if self.severity == "ERROR" else "!" if self.severity == "WARNING" else "i"
...
def validate_metadata(metadata_file="pachyderm_metadata.json"):
    ...
    FIXED: Added semantic validation rules.
    ...
    if gen_count > 20:
        alerts.append(ValidationAlert(
            ...
            f"Generator count ({gen_count}) seems unusually high for a data center",
            "Verify BOD parsing correctly extracted generator count (not word occurrences). Typical data centers have 4-12 generators."
        ))
    elif gen_count == 0:
        alerts.append(ValidationAlert(
            "WARNING", "GENERATORS", "count",
            "No generators detected",
            "Verify BOD contains generator specifications"
        ))

    # Check generator ratings are reasonable
    for gen in metadata.get("generators", []):
        ...
        # Check ratings are reasonable for data center
        if kw < 500:
            alerts.append(ValidationAlert(
                "WARNING", gen["id"], "kw",
                f"Generator rating ({kw} kW) seems small for data center",
                "Typical data center generators are 1-5 MW. Verify BOD parsing extracted correct rating."
            ))
        elif kw > 10000:
            alerts.append(ValidationAlert(
                "INFO", gen["id"], "kw",
                f"Generator rating ({kw} kW) is very large",
                "Verify this is correct for your facility size"
            ))

    # Check transformer count and sizing
    tx_count = len(metadata.get("transformers", []))
    if tx_count > 20:
        alerts.append(ValidationAlert(
            "WARNING", "TRANSFORMERS", "count",
            f"Transformer count ({tx_count}) seems high",
            "Verify BOD parsing correctly extracted transformer count"
        ))

    for tx in metadata.get("transformers", []):
        rating = tx.get("rated_s", 0)
        if rating < 500:  # Most data center transformers are > 500 kVA
            alerts.append(ValidationAlert(
                "WARNING", tx["id"], "rated_s",
                f"Transformer rating ({rating} kVA) seems small for data center",
                "Typical data center transformers are 1,000-5,000 kVA. Verify BOD parsing extracted correct rating (not auto-calculated)."
            ))
        elif rating > 10000:
            alerts.append(ValidationAlert(
                "INFO", tx["id"], "rated_s",
                f"Transformer rating ({rating} kVA) is very large",
                "Verify this matches BOD specifications"
            ))
    ...
    if ups_count > 30:
        alerts.append(ValidationAlert(
            ...
            "Verify BOD parsing correctly extracted UPS count (not word occurrences). Typical data centers have 5-20 UPS modules."
        ))
    elif ups_count == 0:
        alerts.append(ValidationAlert(
            "WARNING", "UPS", "count",
            "No UPS systems detected",
            "Verify BOD contains UPS specifications"
        ))
    ...
    # Check IT vs Mechanical UPS distribution
    it_ups = [u for u in metadata.get("ups", []) if u.get("function") == "IT"]
    mech_ups = [u for u in metadata.get("ups", []) if u.get("function") == "MECHANICAL"]

    if len(it_ups) == 0 and ups_count > 0:
        alerts.append(ValidationAlert(
            "WARNING", "UPS", "function",
            "No IT UPS detected - all UPS marked as MECHANICAL",
            "Verify BOD parsing correctly distinguished IT UPS from Mechanical UPS"
        ))

    if len(it_ups) == ups_count and ups_count > 10:
        alerts.append(ValidationAlert(
            "WARNING", "UPS", "function",
            "All UPS marked as IT - no Mechanical UPS detected",
            "Verify BOD contains mechanical UPS specifications or if system truly has no mechanical UPS"
        ))
    ...
    for swbd in metadata.get("mv_swbd", []) + metadata.get("lv_swbd", []):
        if "redundancy" not in swbd or not swbd["redundancy"]:
            alerts.append(ValidationAlert(
                "WARNING", swbd["id"], "redundancy",
                "Missing or empty redundancy field",
                "Define redundancy as 'A', 'B', 'A/B', 'N+1', or '2N' based on Tier requirements"
            ))
    ...
    if topology == "ring":
        ...
        if rmu_count < 4:
            alerts.append(ValidationAlert(
                "WARNING", "RMU", "count",
                f"Ring bus has only {rmu_count} RMUs (typical: 4-8)",
                "Ensure proper ring segmentation with RMUs at each node"
            ))
        elif rmu_count == 4:
            alerts.append(ValidationAlert(
                "INFO", "RMU", "count",
                f"Ring bus has {rmu_count} RMUs (standard configuration)",
                "Verify this matches BOD specifications. Some designs use 6 or 8 RMUs for dual-ring."
            ))
...
def generate_validation_report(alerts, output_file="validation_report.txt"):
    ...
    report_lines = [
        ...
        f"Validator Version: 2.0.0 (FIXED with semantic validation)",
        "",
        "-" * 70,
        "Validation Summary",
        "-" * 70,
        f"[X] {error_count} Error(s)",
        f"[!] {warning_count} Warning(s)",
        f"[i] {info_count} Info Message(s)",
        "",
        "-" * 70,
...
def main():
    ...
    print("=" * 70)
    print("PACHYDERM GLOBAL - Metadata Validator (FIXED)")
    print("=" * 70)
